...
object DataSeeder {
    ...
    // Separate prefs file so logout (which clears budgie_prefs) doesn't reset this flag
    private const val SEED_PREFS = "budgie_app_state"
    ...
    suspend fun seedIfNeeded(context: Context) {
        ...
        // Use categories already seeded by AppDatabase.onCreate — don't insert duplicates
        val saved = db.categoryDao().getCategoriesSync(guestUserId)
        fun catId(name: String) = saved.firstOrNull { it.name.equals(name, ignoreCase = true) }?.id ?: 1

        val foodId       = catId("Groceries")
        ...
        val transactions = listOf(
            ...
            Transaction(amount = 89.99,   categoryId = foodId,        categoryName = "Groceries",     type = "expense", description = "Woolworths grocery run",          date = daysAgo(0), userId = guestUserId),
            ...
            Transaction(amount = 55.50,   categoryId = foodId,        categoryName = "Groceries",     type = "expense", description = "Lunch at Vida e Caffè",           date = daysAgo(1), startTime = "12:30", endTime = "13:00", userId = guestUserId),
            ...
            Transaction(amount = 75.00,   categoryId = foodId,        categoryName = "Groceries",     type = "expense", description = "Checkers weekly groceries",       date = daysAgo(5), userId = guestUserId),
            ...
            Transaction(amount = 95.00,   categoryId = foodId,        categoryName = "Groceries",     type = "expense", description = "Spur family dinner",              date = daysAgo(18), startTime = "18:00", endTime = "19:45", userId = guestUserId),
        ...
        val goals = listOf(
            BudgetGoal(categoryId = foodId,      categoryName = "Groceries",     minBudget = 200.0,  maxBudget = 600.0,  userId = guestUserId, month = month, year = year),
